...
```
cinc policy-group acl revoke <perm> <name> [flags]
```

### Examples

Revoke a user's update access on policy-group production.

```bash
cinc policy-group acl revoke update production --user alice
```
